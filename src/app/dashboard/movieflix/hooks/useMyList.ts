...
interface UseMyListReturn {
  ...
  addToMyList: (movieId: string) => Promise<'added' | 'duplicate' | 'error'>;
...
export const useMyList = (): UseMyListReturn => {
  ...
  const fetchMyList = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase.from('movie_list').select('movie_id');
      ...
      const movies = await Promise.all((data ?? []).map(row => fetchMovieDetail(row.movie_id)));
  ...
  const addToMyList = useCallback(
    async (movieId: string): Promise<'added' | 'duplicate' | 'error'> => {
      const {
        data: { user }
      } = await supabase.auth.getUser();
  ...
  return { myList, loading, error, fetchMyList, addToMyList, removeFromMyList };
};
