...
const Header = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (keyword.trim()) {
        ...
        if (
          !pathNameRef.current.includes('/search')
        ) {
          previousPath.current =
            pathNameRef.current; // 検索開始時のパスを保存
          sessionStorage.setItem(
            'movieflix:previousPath',
            pathNameRef.current
          ); // キャッシュにも保存
        }
        router.replace(
          `/dashboard/movieflix/search?q=${encodeURIComponent(keyword)}`
        );
        // 文字列がない場合. （検索欄が空になったら）
      } else {
        const back =
  ...
  return (
    <header
      className={toClassNames([styles.appHeader])}
    >
      <h1
        onClick={() => {
