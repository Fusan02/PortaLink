...
export default function Game() {
  ...
  useEffect(() => {
    ...
    for (let row = 0; row < GAME_CONFIG.enemy.rows; row++) {
      for (let col = 0; col < GAME_CONFIG.enemy.cols; col++) {
        ...
        const y =
          ...
          row * (GAME_CONFIG.enemy.height + GAME_CONFIG.enemy.spacing);
    ...
    const level = [
      GAME_CONFIG.enemy.easy,
      GAME_CONFIG.enemy.normal,
      GAME_CONFIG.enemy.hard
    ];
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isCheating) {
        ...
        if (
          e.key === ' ' ||
          (e.key === 'Enter' && currentGameState === GameState.PLAYING)
        ) {
    ...
    const gameLoop = (currentTime: number = 0) => {
      ...
      // 敵がランダムに弾を発射
      enemyShootTimer++;
      if (enemyShootTimer > level[difficulty]) {
        // 難易度に応じて撃つ頻度変更
        enemyShootTimer = 0;
        ...
        if (aliveEnemies.length > 0) {
          const randomEnemy =
            aliveEnemies[Math.floor(Math.random() * aliveEnemies.length)]; //ランダムに敵から１匹選ぶ
      ...
      if (isMovingDown) {
        ...
        enemies.forEach(enemy => {
          ...
          if (
            enemyDirection > 0 &&
            enemy.position.x + enemy.size.width >= canvas.width
          ) {
            hitEdge = true;
          }
