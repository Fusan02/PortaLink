        ...
        <FadeInAnimation>
            <div className={page}>
                ...
                <Texts
                    ...
                    text={`登録日: ${new Date(profile.created_at).toLocaleString('ja-JP')}`}
                />
                <Texts
                    className={styles.time}
                    text={`最終更新: ${new Date(profile.updated_at).toLocaleString('ja-JP')}`}
                />
                <Button
                    className={toClassNames([styles.button, boxAnimation])}
                    text='プロフィールを更新'
                    onClick={() =>
                        router.push('/dashboard/profile/profile_setting')
                    }
                />
